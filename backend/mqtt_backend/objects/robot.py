    ...
    def handle_message(self, message):
        ...
        if handler:
            if message['protocol'] == "DICOM":
                ...
                print(f"[SurgicalRobot:{self.object_id}] Got DICOM image ({len(binary)} bytes)")
            elif message['protocol'] == "HL7":
                hl7_str = handler.decode(message['payload'])
                print(f"[SurgicalRobot:{self.object_id}] Got HL7 message: {hl7_str}")
        else:
            print(f"[SurgicalRobot:{self.object_id}] Received message: {message}")
